...
# -*- coding: utf-8 -*-
"""
Приём команд из OPC UA и запись в очередь
"""

class OpcCommandReceiver:
    """
    Приём команд из OPC UA и запись в commands_queue

    Это простой класс без потоков! Вызывается синхронно из callback'а.
    """

    def __init__(self, db: Database):
        self.db = db
        self.logger = logging.getLogger('opc.commands.receiver')
    ...
    def queue_command(
            ...
            sim: str,
            params: dict,
            requested_by: str = 'opc_user'
    ) -> int:
        """
        Добавляет команду в очередь

        Returns:
            int: ID записи в очереди или -1 при ошибке
        """
        try:
            # Получаем command_id
            cmd_rows = self.db.query("""
                SELECT id FROM commands_catalog 
            ...
            command_id = cmd_rows[0][0]

            # Записываем в очередь
            rows = self.db.query("""
                INSERT INTO commands_queue 
                (command_id, sim, params, status, requested_by, created_at)
                VALUES (%s, %s, %s, 'pending', %s, NOW())
                RETURNING id
            """, (command_id, sim, json.dumps(params, ensure_ascii=False), requested_by))

            queue_id = rows[0][0]
            self.logger.info(f"✅ Команда {command_code} в очереди: ID={queue_id}")

            return queue_id

        except Exception as e:
            self.logger.error(f"❌ Ошибка записи в очередь: {e}", exc_info=True)
            return -1

    # ❌ НЕТ метода start() / stop() - не нужен!
    # Этот класс вызывается синхронно из OPC UA callback
# class OpcCommandReceiver:
